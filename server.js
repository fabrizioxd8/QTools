...
pem.createCertificate({ days: 365, selfSigned: true }, (err, keys) => {
    ...
    (async () => {
        ...
        const toolCount = await db.get('SELECT COUNT(*) as count FROM tools');
        if (toolCount.count === 0) {
            console.log('Database is empty. Seeding with initial data...');
            await db.run("INSERT INTO tools (name, category, status, isCalibrable, calibrationDue, image, customAttributes) VALUES ('Digital Multimeter', 'Electrical', 'Available', 1, '2025-12-31', '/placeholder.svg', '{\"brand\":\"Fluke\",\"model\":\"87V\"}')");
            await db.run("INSERT INTO tools (name, category, status, isCalibrable, calibrationDue, image, customAttributes) VALUES ('Torque Wrench', 'Mechanical', 'Available', 1, '2025-11-15', NULL, '{\"range\":\"10-150 Nm\"}')");
            await db.run("INSERT INTO workers (name, employeeId) VALUES ('John Smith', 'EMP001')");
            await db.run("INSERT INTO workers (name, employeeId) VALUES ('Sarah Johnson', 'EMP002')");
            await db.run("INSERT INTO projects (name) VALUES ('Building A Renovation')");
            await db.run("INSERT INTO projects (name) VALUES ('Lab Equipment Installation')");
        ...
        try {
            ...
            const assignments = await Promise.all(assignmentsData.map(async (a) => {
                ...
                const toolsInAssignment = await Promise.all(assignmentTools.map(async (at) => {
                    const toolData = await db.get('SELECT * FROM tools WHERE id = ?', at.toolId);
                    const tool = { ...toolData, customAttributes: JSON.parse(toolData.customAttributes || '{}') };
            ...
            const parsedTools = tools.map(tool => ({
                ...tool,
                customAttributes: JSON.parse(tool.customAttributes || '{}')
            }));

            res.json({ tools: parsedTools, workers, projects, assignments });
        } catch (err) {
